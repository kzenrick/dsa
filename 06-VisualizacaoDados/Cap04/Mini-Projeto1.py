# Mini-Projeto 1 - Dashboard Interativo com Streamlit, Folium e Plotly Para
# Monitoramento de Casos de Covid-19 em Tempo Real
...
def main():
    ...
    st.markdown("<h1 style='text-align: center;color: #fa634d;'>"\
                "<strong><u>Real-Time Covid-19 Dashboard</u></strong></h1>",
                unsafe_allow_html = True)
    
    # Título do menu lateral
    st.sidebar.markdown("<h1 style='text-align: center; color: #baccee;'>"\
                        "<strong>" \
                        "<u>Monitoramento de Casos de Covid-19</u>"\
                        "</strong>"\
                        "</h1>", 
                        unsafe_allow_html = True)

    # Sub-títulos da área principal
    st.markdown("O Dashboard Utiliza Dados Reais da Johns Hopkins CSSE.", 
                unsafe_allow_html = True)
    
    st.markdown("Os Dados São Atualizados Diariamente.", 
                unsafe_allow_html = True)
    ...
    removeds = ['NewConfirmed', 'NewDeaths', 'NewRecovered']
    dfn = covid1.drop(removeds, axis = 1)
    
    grouped = ['TotalConfirmed','TotalDeaths','TotalRecovered','ActiveCases']
    dfn = dfn.groupby('Country')[grouped].sum().\
        sort_values(by = 'TotalConfirmed', ascending = False)
        
    dfn.style.background_gradient(cmap = 'Oranges')
    dfc = covid1.groupby('Country')[grouped].max().\
        sort_values(by = 'TotalConfirmed', ascending = False).reset_index()
    ...
    st.write(f'Total de Casos Confirmados no Mundo - {confirmed_tot:n}')
    st.write(f'Total de Mortes no Mundo - {deaths_tot:n}')
    st.write(f'Total de Pessoas Recuperadas no Mundo - {recovered_tot:n}')
    st.write(f'Total de Casos Ativos no Mundo - {active_tot:n}')
