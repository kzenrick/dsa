...
# Substituindo espaço por undeline nos nomes muito longos --------
...
# Agrupamento das colunas dos jogadores ---------
...
tmp = Matches %>% 
  ...
  )%>% # Transformas as colunas em linhas
  gather(
    player,                       # Nome da Coluna
    player_api_id,                # Valor da Coluna
    -c(season, home_team_api_id)  # Colunas que se manterão
  ) %>%
...
playerClubSequence$seqnr = ave( playerClubSequence$player_api_id, 
                                playerClubSequence$player_api_id, 
                                FUN = seq_along)
...
write_delim( 
  ...
  delim ="\t", 
  path = "player_transactions.txt", 
  col_names = FALSE
)


# Importamos as transações registradas em arquivo -------
playerstrxs <- read_baskets("player_transactions.txt",
                            sep = "[ \t]+",
                            info =  c("sequenceID","eventID","size") # Agrupamento para determinar a 4%
                            )
summary(playerstrxs)


# Executar mineração de sequência --------
# por enquanto apenas com comprimento de duas sequências
?cspade
...
# para colocar os resultados do cspade em um organizado conjunto de dados 
...
seqResultOriginal = as(playersClubSeq, "data.frame")
seqResult = seqResultOriginal %>% 
  mutate(
...
head(seqResultOriginal, 5)
head(seqResult, 5)

seqResult %>% 
  select (sequence) %>%
  filter(grepl(pattern = 'União', x = sequence))
...
nodesData = data.frame(id = unique(nodes), 
                       title = unique(nodes), 
                       label = unique(nodes), 
                       stringsAsFactors = FALSE) %>%
