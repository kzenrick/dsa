...
# Plotando um gráfico básico com gplot()
...
# Scatter plot com modelo de regressão
# Dados
...
# Outro aspecto que pode ser mapeado é a cor dos pontos
ggplot(data = mtcars,
...
# Os geoms definem qual forma geométrica será utilizada para
# a visualização dos dados no gráfico
ggplot(data = mtcars,
...
ggplot(mtcars,
       aes(x = as.factor(cyl), y = mpg, colour = as.factor(cyl))) +
  geom_boxplot()
...
ggplot(mtcars,
       aes(x = as.factor(cyl), y = mpg)) +
  geom_boxplot(color = c("green", "pink", "blue"), fill = c("chocolate", "tomato", "seagreen4"))
...
# Legendas
ggplot(mtcars, aes(x = as.factor(cyl), fill = as.array(cyl))) +
  geom_bar() +
  labs(fill = "cyl")

# Trocando posição da legenda
ggplot(mtcars,
...
# Combina os 2 plots na mesma área <Grid>
