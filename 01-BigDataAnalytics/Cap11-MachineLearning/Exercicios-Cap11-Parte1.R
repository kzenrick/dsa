...
lines(x, y3, lty=2, col = 'red')
lines(x, y4, lty=4, col = 'yellow')
lines(x, y5, lty=5, col = 'pink')
...
Tempo <- c(17.87, 13.75, 12.72, 6.98, 11.01, 10.48, 10.19, 19.11, 12.72, 0.45, 
           10.67, 1.59, 14.91, 14.14, 9.40, 16.23, 12.74, 20.64, 12.34, 6.44)

# Juntando os dados em 1 data frame
df <- data.frame(Idade, Tempo)
names(df)
...
rg <- lm(Idade ~ Tempo, data = df)
rg1 <- lm(Tempo ~Idade, data = df)
...
camada2 <- geom_smooth(mapping = aes(x=Idade, y=Tempo),
                       data = df,
                       method = lm, 
                       col = 'red')
...
pesos2 <- predict(modelo, alturas2)
pesos2

df1 <- data.frame(alturas2[1], pesos2)
names(df1) <- c("Altura", "Peso")

plot(df1$Altura, df1$Peso, 
     pch = 16, 
     cex = 1.3, 
     col = "red", 
     main = "Altura x Peso", 
