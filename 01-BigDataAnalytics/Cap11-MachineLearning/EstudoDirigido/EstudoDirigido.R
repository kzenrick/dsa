...
setwd("~/Documentos/dsa/BigDataAnalytics/Cap11-MachineLearning/EstudoDirigido")
getwd()
...
## Problemas ao rodar o C50 com a versão mais atual
# Remover a atual
# remove.packages("C50", lib="~/anaconda3/envs/dsa/lib/R/library")

# Instalar a específica
# install.packages("C50", version = "0.1.0", repos = "http://cran.us.r-project.org")
# https://cran.r-project.org/src/contrib/Archive/C50/
...
# Converte os valores para inteiros * 10^10
if (F){
  dados1 <- dados
  
  for (i in 1:28){
    col = paste('V', i, sep='')
    dados1[[col]] <- as.integer(dados1[[col]] * (10 ** 5))
  }
  
  dados1$Amount <- as.integer(dados1$Amount * 100)
...
# Converte os dados de treino e teste para numérico

for (i in 1:28){
  col = paste('V', i, sep='')
  rose_teste[[col]] <- as.integer(rose_teste[[col]])
  rose_treino[[col]] <- as.integer(rose_treino[[col]])
}

rose_teste$Amount <- as.integer(rose_teste$Amount)
rose_teste$Time <- as.integer(rose_teste$Time)

rose_treino$Amount <- as.integer(rose_treino$Amount)
rose_treino$Time <- as.integer(rose_treino$Time)
