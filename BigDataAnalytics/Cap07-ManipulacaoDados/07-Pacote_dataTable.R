setwd("~/Projetos/Python/git/R/DSA/BigDataAnalytics/Cap07-ManipulacaoDados")
getwd()

library(data.table)

# criando 2 vetores
vec1 <- c(1,2,3,4)
vec2 <- c('vermelho', 'verde', 'marrom', 'laranja')

# criando uma data.table
?data.table
dt1 <- data.table(vec1, vec2)
dt1
class(dt1)

# slicing do data.table
dt2 <- data.table(A = 1:9, B = c('Z', 'W', 'Q'), C = rnorm(9), D = T)
dt2
dt2[3:5,]
dt2[, .(B,C)] # . representa a sintaxe

# Aplicando a função ao data.table
dt2[, .(Total = sum(A), Mean = mean(C))]
dt2[, plot(A,C)]
# Usando um agrupamento
dt2[, .(MySum = sum(A)), by = .(Grp = A%%2)]

# Definindo os valores por grupos
dt3 <- data.table(B = c('a', 'b', 'c', 'd', 'e', 'a', 'b', 'c', 'd', 'e'),
                  val = as.integer(c(6:10, 1:5)))
dt3

# Operações com data tables
dt4 <- data.table(A = rep(letters[2:1], each = 4L),
                  B = rep(1:4, each = 2L),
                  C = sample(8))
dt4

# Soma valores de C agrupando por A
new_dt4 <- dt4[, sum(C), by = A]
class(new_dt4)
new_dt4
new_dt4[order(A)]

# Soma B agrupado por A ordenado por A 
new_dt4 <- dt4[, sum(B), by = A][order(A)]
new_dt4

# Iris
dt5 <- as.data.table(iris)
dt5
dt5[, .(Sepal.Length = median(Sepal.Length),
        Sepal.Width = median(Sepal.Width),
        Petal.Length = median(Petal.Length),
        Petal.Width = median(Petal.Width)
        ),
    by = Species]

# O Comando .SD significa Subset Data e um subset é criado
# considerando a coluna é calculada a mediana.
dt5[, lapply(.SD, median), by = Species]
  