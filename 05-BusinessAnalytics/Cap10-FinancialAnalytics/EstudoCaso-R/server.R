setwd("~/Documentos/dsa/05-BusinessAnalytics/Cap10-FinancialAnalytics/EstudoCaso-R")
library(shiny)
...
#---------------------------------- Tab Modelo Black-Litterman ----------------------------------------
  output$ui <- renderUI({
    switch(input$selectType,
      "relative" = selectInput("newAssetClass", label = "Escolha Outra Classe de Ativos", choices = assetClasses[-which(assetClasses==input$assetClass)])
    )
  })
